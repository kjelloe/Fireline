...
case "${1:-}" in
  ...
  map)    $AM queue add --for batch-pc --as dev --body "{\"kind\":\"map\",\"map\":\"${2:?usage: batch_send.sh map <profile> [count] [mirror] [uniques] [swap] [stalemate]}\",\"count\":${3:-300},\"mirror\":${4:-0},\"uniques\":${5:-0},\"swap\":${6:-0},\"stalemate\":${7:-1},\"orderparity\":${8:-0},\"handicap\":\"${9:-}\",\"slide\":${10:-1},\"standoff\":${11:-1}}" ;;
