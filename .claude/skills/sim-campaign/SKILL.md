...
|---|---|---|
| frontier DEFAULT (post-sweep, 72dae18) | **42.8 / 44.0% A** (n=600) — BELOW BAND, team-keyed B edge ~6.6 pts; the drift arrived with the obstacle-slide era (46.2/45.5 → 43.1/45.6 → 42.8/44.0) | **TOP OPEN ITEM** — CENSUS SHAPE known (2026-08-03): B penetrates deep-west and survives (17 vs 2-3, both configs); A loses the centre 2:1 under POWS, soft-skins 1.6x. Next: killer-team census; VERIFY aiMirrored's patrol swap end-to-end (a pure patrol lean should flip in mirror pairs and doesn't — a broken swap would also void mirror-battery semantics); SLIDE=0 pair in flight; the t=5,444 thread |
...
| riverline (CURRENT same-build pair, 269897d) | **50.8 / 41.0% A** (agg 45.9) | The escort-wall era MOVED riverline: now team-fair-ish with a SIDE-KEYED east curse (~9 pts — the same magnitude as convoy's east-pusher residual). NO OFFSET (side-keyed leans flip with the join — the premium generator's own law); premium disclosure stays. THE EAST CURSE (~9 pts, riverline + convoy) is the one remaining side-keyed mystery |
