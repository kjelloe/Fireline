...
13E BRIDGES (2026-07-29):  slice-13e-bridges ✅ (hashed spans, artillery breach -> span becomes WATER so the Skimmer answers a demolition, either-team repair, NO fixture repin) + slice-13e2 ✅ (AI doctrine: drop only when LOSING the crossing, one besieger per span, truck rebuild errand; probed 3/3 wars) · perf_native runner ✅ · batch lane observability ✅ (auto-mail reports + 5 silent failures made loud)
B1 + PLAYTEST 9 (2026-07-29): slice-b1 ✅ meaningful deaths (600-war verdict: horn 27→13%, standard held 8%, fair 51-53%) · playtest-9 ✅ (carrier-passenger STRANDING fixed — disable never released aboard seats; out-of-range hover; mission cards ranked by Recognition value then locality; resupply cards) · map selector ✅ · perf -Uncapped ✅ · SAWTOOTH 600-war battery OVERTURNS the faction verdict: it is an EAST SIDE lean (geometry), and blackwood leans WEST — 18F is a side-lean hunt
2026-07-30 COORDINATE FIX: 18F hunt found the INSTRUMENT not the maps — mirror transform sent the east edge off-map and mid-cell positions one cell west; 180-degree turn tie was chirally fixed; and the root blocker was the LEFT-EDGE convention (a left edge cannot reflect onto a left edge). cellToWorld -> c*256+128 (cell CENTRES), fixture v41, mirror divergence tick 2 -> 54-166 with a sub-cell residual. CONSEQUENCE: every pre-2026-07-30 balance number is VOID; re-measurement battery queued. Worker `resync` added after the battery started on a 7-commit-old build (upstream rebase = permanent divergence).
2026-07-30 DESIGNER ROUND: rulings recorded (52/48 tolerate 54/46 · Skimmer mobility lever not capture-speed · 20-22 min target · blackwood terrain-only recovery + banked winch/depot/ranger · hold sawtooth for human playtest) · field repair ✅ (capped half hull + AI doctrine) · slice-b3 ✅ (mercy RETARGETED — ruled trigger unreachable; cured blackwood) · slice-34 waypoints ✅ · STORY INSTRUMENT ✅ (war_metrics + analyze_story + TICKETPOOL) · pool probes: 350 and 375 both hit 20-22min/15% horn at n=20, battery decides · NEXT: pool battery → Skimmer retune → blackwood corridors → B4/B7 (need repin: deed counters + killer-in-disable-event)
